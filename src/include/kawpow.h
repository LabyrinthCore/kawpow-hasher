...
struct kawpow_epoch_context
{
    ...
    const union kawpow_hash512* const light_cache;
 ...
 */
union kawpow_hash256 kawpow_calculate_epoch_seed(int epoch_number) NOEXCEPT;
...
struct kawpow_result kawpow_hash(const struct kawpow_epoch_context* context,
    const union kawpow_hash256* header_hash, uint64_t nonce) NOEXCEPT;
...
bool kawpow_verify_final_hash(const union kawpow_hash256* header_hash,
